...
-- INFO. PORCEDURE
    ...
    INFVERSION   VARCHAR := '2024.02.16.03';
...
BEGIN
    ...
    INFPROCESO := '00400';
    IF VPCRUD != 'C' THEN
        SELECT COUNT(1) = 0 INTO VLERROR FROM CATEGORIAS WHERE CLAVE = VLCATEGORIA.CLAVE;
        IF VLERROR THEN
    ...
    INFPROCESO := '00600';
    IF VPCRUD = 'U' THEN
    RAISE EXCEPTION '%', ||LENGTH(VLCATEGORIA.CLAVE);
    ...
    IF VPCRUD = 'D' THEN
        ...
        INFPROCESO := '00710';
        SELECT COUNT(1) > 0 INTO VLERROR FROM GASTOS WHERE CATEGORIA = VLCATEGORIA.CLAVE;
        IF VLERROR THEN
            RAISE EXCEPTION 'No es posible eliminar la categoria' ;
        END IF;
